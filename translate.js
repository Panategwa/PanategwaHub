...
async function translatePage(lang) {
  ...
  for (const node of nodes) {
    ...
    // Always reset from the locked original text first.
    applyText(node, original);

    if (lang === "en") continue;

    // Exact manual whole-node match.
...
function withLang(urlString, lang) {
...
function patchOnclick(code, lang) {
  let out = code;

  out = out.replace(
    /(window\.location\.href\s*=\s*['"])([^'"]+)(['"])/g,
    (_, pre, url, post) => `${pre}${withLang(url, lang)}${post}`
  );
...
function syncNavigationForLanguage() {
  ...
  document.querySelectorAll("a[href]").forEach(a => {
    ...
    const originalHref = a.dataset.originalHref;
    const fixed = isEnglish ? originalHref : withLang(originalHref, lang);
  ...
  document.querySelectorAll("button[onclick]").forEach(btn => {
    ...
    const fixed = isEnglish ? originalOnclick : patchOnclick(originalOnclick, lang);
